...
let nom, contrasenya;
let scriptURL = "https://script.google.com/macros/s/AKfycbz3fpX0hR13KgIqXeBlmhRppY1gNt4mTDMLyYbqq2mHMNX593ZxE8Cyn-_dm4BKti5a/exec"    // s'ha de substituir la cadena de text per la URL del script
...
function canvia_seccio(num_boto) {
    ...
    for (let i = 1; i < num_botons; i++) {
        ...
        if (i == num_boto) {
            boto.style.color = "#000000";    // es destaca la secció activa amb el canvi de colors del botó corresponent
            boto.style.backgroundColor = "#868686";
            seccio.style.display = "flex";    // es fa visible la secció activa
        }
        else {
            boto.style.color = "white";    // colors dels botons de seccions inactives
            boto.style.backgroundColor = "#000000";
